# Migrated from approach1_tabpfn_bash_fresh_fit_probs.py (Chapter 4, Section 4.3).
...
HISTORY_WINDOW_MONTHS = 24       # Set the history window in months  # try 12, 24, 36, 1000 as well
...
def main():
    ...
    MIN_BONDS_PER_CLASS = 10

    # --- 1. Fresh Transitions in Window [t - HISTORY_WINDOW_MONTHS, t) ---
    ...
    # --- 2. Causal Quiescent Anchors at (t - 1m) ---
    # Identify bonds that didn't change rating in the 12m prior to date
    hist_lookback = dataset[
        (dataset["dates"] < date) &
        (dataset["dates"] >= (pd.to_datetime(date) - pd.DateOffset(months=12)))
    ]
    transitions_per_bond = hist_lookback.groupby('bond_id')['is_fresh_rating'].sum()
    quiescent_bond_ids = transitions_per_bond[transitions_per_bond == 0].index

    quiescent_anchors = dataset[
        (dataset["dates"] == (pd.to_datetime(date) - pd.DateOffset(months=1))) &
        (dataset["bond_id"].isin(quiescent_bond_ids))
    ].copy()

    # --- 3. Initial Baseline Assembly ---
    base_historical = pd.concat([fresh_window, quiescent_anchors], axis=0).drop_duplicates()

    # --- 4. Enforce Minimum 5 Bonds Per Class (Look Back dynamically) ---
    all_possible_classes = np.unique(dataset[TARGET].dropna())
    
    # Pool of ALL valid prior data (both fresh and quiescent) strictly before current date
    prior_pool = dataset[
        (dataset["dates"] < date) & 
        (dataset["is_fresh_rating"] == True)  # prioritize fresh transitions first
    ].copy()

    additional_rows = []
    
    for cls in all_possible_classes:
        # Count how many bonds of this rating exist in base_historical
        existing_count = (base_historical[TARGET] == cls).sum()
        needed = MIN_BONDS_PER_CLASS - existing_count

        if needed > 0:
            # Find candidate rows for this class prior to current_date
            cls_candidates = prior_pool[prior_pool[TARGET] == cls]
            
            # Exclude rows already present in base_historical (by index or bond_id + date)
            cls_candidates = cls_candidates[~cls_candidates.index.isin(base_historical.index)]

            if not cls_candidates.empty:
                # Sort by date descending (most recent first) and take up to 'needed' rows
                top_needed = cls_candidates.sort_values("dates", ascending=False).head(needed)
                additional_rows.append(top_needed)

    # --- 5. Final Assembly ---
    if additional_rows:
        backfill_df = pd.concat(additional_rows, axis=0)
        historical_data = pd.concat([base_historical, backfill_df], axis=0).drop_duplicates()
    else:
        historical_data = base_historical

    print(f"Final Train Class Distribution:\n{historical_data[TARGET].value_counts()}")
    ...
    file_path = f"{OUT_DIR}/tabpfn_data_{TARGET}_{HISTORY_WINDOW_MONTHS}_fresh_fit_minbonds_{MIN_BONDS_PER_CLASS}.csv"
