# Migrated from approach1_tabpfn_bash_delta_fit_probs_dom.py (Chapter 4, Section 4.3).
...
TARGET = "rtg"                   # Base rating variable
HISTORY_WINDOW_MONTHS = 36       # Historical window K in months
MIN_HISTORY_WINDOW_MONTHS = 36   # Minimum required history in months before running evaluation
MIN_BONDS_PER_DELTA_CLASS = 5    # Minimum context rows per rating delta category
MAX_DOMINANT_CLASS_RATIO = 0.5  # Cap any single rating delta class to at most 50% of total train rows
if TARGET == "rtg":
...
def get_next_date_str(all_dates, current_date_dt):
    """Finds the next chronological date string or 'STOP' if completed."""
...
def cap_dominant_class_imbalance(df, max_ratio=MAX_DOMINANT_CLASS_RATIO):
    """
    Caps any class exceeding max_ratio of total dataset size.
    Prioritizes fresh ratings (is_fresh_rating == True) and sporadically samples stable rows.
    """
    if df.empty:
        return df

    total_rows = len(df)
    class_counts = df['rating_delta'].value_counts()
    
    # Check if any class violates the max ratio
    dominant_classes = [cls for cls, count in class_counts.items() if (count / total_rows) > max_ratio]
    ...
    for cls in df['rating_delta'].unique():
        cls_df = df[df['rating_delta'] == cls]
        
        if cls in dominant_classes:
            other_rows_count = total_rows - len(cls_df)
            # Calculate max allowed count for this dominant class
            max_allowed = int((max_ratio / (1.0 - max_ratio)) * other_rows_count)
            max_allowed = max(MIN_BONDS_PER_DELTA_CLASS, max_allowed)
            
            fresh_cls = cls_df[cls_df['is_fresh_rating'] == True]
            stable_cls = cls_df[cls_df['is_fresh_rating'] == False]
            
            if len(fresh_cls) >= max_allowed:
                # If fresh ratings alone exceed max_allowed, keep all fresh ratings to preserve transition signals
                kept_indices.extend(fresh_cls.index.tolist())
            else:
                # Keep all fresh ratings and sporadically sample stable ratings to fill the remaining quota
                needed_stable = max_allowed - len(fresh_cls)
                kept_indices.extend(fresh_cls.index.tolist())
                
                if len(stable_cls) > 0 and needed_stable > 0:
                    # Sporadic (evenly spaced) temporal sampling of stable rows
                    sample_positions = np.linspace(0, len(stable_cls) - 1, num=min(needed_stable, len(stable_cls)), dtype=int)
                    sampled_stable = stable_cls.iloc[sample_positions]
                    kept_indices.extend(sampled_stable.index.tolist())
        else:
            kept_indices.extend(cls_df.index.tolist())

    return df.loc[df.index.isin(kept_indices)].copy()
...
def main():
    ...
    # All unique dates sorted for bash control
    ...
    fresh_window = dataset[
        (dataset["dates"] < current_date_dt) &
        (dataset["dates"] >= hist_start) &
        (dataset["is_fresh_rating"] == True)
    ].copy()

    # B. Post-Switch Stable Datapoints (N/2 post-switch snapshots):
    # For bonds that switched ratings at t_switch < t and remained unchanged for N months, take up to N/2 snapshots
    ...
    post_switch_rows = []
    for bond_id, b_df in hist_window_all.groupby('bond_id'):
        b_df = b_df.sort_values('dates')
        fresh_indices = b_df[b_df['is_fresh_rating'] == True].index.tolist()
        
        for idx in range(len(fresh_indices)):
            start_idx = fresh_indices[idx]
            end_idx = fresh_indices[idx+1] if idx + 1 < len(fresh_indices) else b_df.index[-1] + 1
            segment = b_df.loc[start_idx:end_idx-1]
            
            n_months = len(segment)
            if n_months > 1:
                # Take up to N/2 post-switch stable snapshots (skipping the 0-th fresh row already in fresh_window)
                take_k = max(1, n_months // 2)
                stable_post = segment.iloc[1:1+take_k]
                if not stable_post.empty:
                    post_switch_rows.append(stable_post)

    post_switch_df = pd.concat(post_switch_rows, axis=0) if post_switch_rows else pd.DataFrame(columns=dataset.columns)

    # C. Quiescent Anchor Regime (Bonds with ZERO rating changes in full window)
    ...
    base_frames = [df for df in [fresh_window, post_switch_df, quiescent_oldest_half] if not df.empty]
    base_historical = pd.concat(base_frames, axis=0).drop_duplicates() if base_frames else pd.DataFrame(columns=dataset.columns)

    # Minimum Delta Class Enforcement (-5 to +5 full spectrum backfill)
    prior_pool = dataset[
        (dataset["dates"] < current_date_dt) & 
        (dataset["is_fresh_rating"] == True)
    ].copy()
    ...
    if additional_rows:
        ...
        raw_historical_data = pd.concat(hist_frames, axis=0).drop_duplicates()
    else:
        raw_historical_data = base_historical

    historical_data = cap_dominant_class_imbalance(raw_historical_data, max_ratio=MAX_DOMINANT_CLASS_RATIO)
    ...
    out_dir = OUT_DIR
    os.makedirs(out_dir, exist_ok=True)
    file_path = os.path.join(out_dir, f"tabpfn_delta_fit_dom_{MAX_DOMINANT_CLASS_RATIO*100:.0f}_{TARGET}_{HISTORY_WINDOW_MONTHS}m.csv")
    ...
    # Output next date for outer bash runner
    print(next_date_str)
